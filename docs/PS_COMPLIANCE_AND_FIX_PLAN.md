...
|---|---|---|---|
| 1 | Parse bank statements (Excel/PDF/CSV) | 🟡 works, lossy — **loss now itemised** | **140** BANK tables → **39,170 txns** (was 122 / 36,281). Fixed-width print statements and broken-geometry portal PDFs now read. Of 117,741 rejected rows on `fir-65-2024`, **34.6% are de-duplicated events and 0.4% blank layout rows** — neither is a loss — and 13.7% are unclaimed tables (FR-4, 82% out of scope). The one actionable category, `row missing timestamp / primary identifier`, was **one reason hiding two**: a row with no timestamp can never be an event, while a row that has a time and lost its identifier is a mapping gap. Now split |
...
| 4 | Schema mapping / auto-detection | 🟢 **re-measured 31 Jul — the figure was the wrong unit** | The "658 of 951" counts **tables**, which average 24 rows against thousands in the claimed ones. By **rows**: **95.6%** claimed on `fir-65-2024`, **97.8%** on `FIR-0006-2025 U`. New `unrecognised_by_reason` splits the residue: across both cases the genuine parser gap is **3 tables / 92 rows = 0.008%** of 1,114,168 rows. The rest is out of scope (a CCTV log is 70% of one case's residue), **officer-bearing registers refused on purpose** (337 tables on the other — the `master - Copy.xlsx` shape), or reference data with no time anchor. See COMPONENT_STATUS §2.0 |
...
| 6 | Unified entity model | 🟡 partial, **improving** | **6,681** entities (was 4,182). **ACCOUNT_NO+PHONE 2 → 30** now the Gujarati bank-reply bridge is on by default (35 links, all three safety checks passed — COMPONENT_STATUS §4.1). 25,695 counting external counterparty singletons — the two figures are not interchangeable, see §7.7 |
...
| 13 | Mule-account signatures | 🟡 **eligible = 6, not 9,996** | Counterparty-side flows fixed, so rules can now see an account visible only as somebody else's payee: `mule_account` eligibility on `demo` 30 → 64. It still fires 0 on both real cases, and that is **correct on the evidence** — a counterparty-only entity is a terminal payee, so `max_rapid_forward` is 0 by definition. Six entities in 7,358 reach fan-in ≥ 5; none forwards. The fix's actual yield was elsewhere: **`structuring` 9 → 13 on `FIR-0006-2025 U`**, 4 counterparty accounts each receiving ≥3 credits in [₹9L, ₹10L) **within 24h** — the accounts money was structured *into*. First measured as 14; the other 10 were artefacts of D8 and are withdrawn |
| 14 | Money-flow + comms graphs, drill-down | 🟢 works | 8,435 nodes on real data, HTTP 200 |
| 15 | Filter / search (entity, amount, time, location) | 🟢 **works, extended 31 Jul** | all four on **both** paths: the DSL via `/v1/query`, and `/v1/events` directly (`entity`, `location`, `min_amount`/`max_amount`, `start`/`end`). Plus `GET /v1/document-mentions/{ds}?identifier=` over the **narrative paperwork**: 123 Gujarati police documents indexed by the identifiers they name — 469 accounts, 435 phones, 280 IFSC, 20 IMEI, 11 UPI — of which **39 carry their evidence in prose only** and were invisible to any table reader. Matched as identifiers, not substrings; carries no merge keys (COMPONENT_STATUS §4.3) |
...
Ten defects, of which **one was a regression introduced by the fix for another** and caught
before merge by A/B rather than by a test, and **three were thresholds declared in config that no
code ever read** — found only because a new finding was checked against the rule that produced it. Full detail in `docs/COMPONENT_STATUS.md` §6.1–6.4.
...
| D3 | Counterparty-only entities had empty feature vectors, so no rule could fire on an account seen only as somebody else's payee | `features.build` fills fan-in and transfer-derived flows | `mule_account` eligibility on `demo` 30 → 64. On `FIR-0006-2025 U` `structuring` reached **13 (from 9)** — 4 counterparty accounts receiving ≥3 INR credits in [₹9L, ₹10L) within 24h. This, not `mule_account`, was the yield. **First measured as 14; 10 were artefacts of D8 and are withdrawn** |
...
| D8 | `structuring.window_hours: 24` was **declared in config and never read** — the timestamp was discarded, so every in-band credit in the case counted however far apart. Three ₹9.5-lakh receipts *years* apart read as smurfing | Sliding window; the flag names the burst count and the wider in-band total | `demo` 3 → **3**, precision **1.000**, recall **1.000**. `FIR-0006-2025 U` **23 → 13**: all 10 dropped were counterparty-derived, and **all 9 firings predating D3 survive untouched** |
| D9 | `rapid_in_out.max_hold_minutes: 60` was read **only to build the flag text** while the measurement used a scalar fixed at 120 — so every flag asserted "forwarded within 60min" about a computation that had allowed 120. **The only one of the ten that was wrong on the page** | Each rule computes its own window; `mule_account` keeps 120 | `demo` 28 → **23** fired, precision 0.107 → **0.130**, recall **1.000** — five false positives removed, all three true ones kept. `FIR-0006-2025 U` 58 → **58** |
| D10 | `call_transfer_coincidence.window_minutes: 10` was read by **nothing**; the window is applied upstream in correlation, so editing it changed nothing. A tunable that silently does nothing invites widening the window in the wrong file and concluding the evidence is absent | Removed, with the reason and the real setting recorded in its place | Guarded by a test that reads the YAML and fails if **any** declared threshold is unreferenced by its own rule |
...
**290 tests pass**; `test_ml_fit_population.py` pins D4.
